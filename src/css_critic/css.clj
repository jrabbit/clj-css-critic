...
;;https://github.com/simonhicks/css-parser/blob/master/src/css/core.clj

; TODO remove some of the duplication for colon being used as property/value separator

(def str-trim
  (comp s/ltrim s/rtrim))
...
(def css-attribute-modifier-value-description
  (semantics (conc (opt (alt pipe-char tilde)) equals-sign (alt string-literal (rep+ (except anything close-square-bracket))))
             flat-str))
...
(def css-selector-list
  (complex [first-selector css-selector
    ...
    (make-node :selector-list (filter (comp not nil?) (flatten (list first-selector rest-selectors))))))
